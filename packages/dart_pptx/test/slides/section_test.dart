...
void main() {
  group('Section slide tests', () {
    ...
    test('Check if added', () async {
      ...
      expect(slide.runtimeType, SlideSection);
    });

    test('ppt/slides/slide1.xml', () async {
      const section = 'SLIDE_SECTION';
      String content = getXml(files['ppt/slides/slide1.xml']!);

      expect(content, contains('<p:sld'));
      expect(content.indexOf(section), -1);

      final slide = pres.slides[0] as SlideSection;
      slide.section = TextValue.uniform(section);

      await save();
      content = getXml(files['ppt/slides/slide1.xml']!);

      expect(content.indexOf(section), isNot(-1));
    });
    ...
    test('ppt/slides/_rels/slide1.xml.rels', () {
      ...
      expect(content, contains('slideLayout7.xml"'));
    });
