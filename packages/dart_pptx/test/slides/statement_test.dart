...
void main() {
  group('Statement slide tests', () {
    ...
    test('Check if added', () async {
      ...
      expect(slide.runtimeType, SlideStatement);
    });

    test('ppt/slides/slide1.xml', () async {
      const statement = 'SLIDE_STATEMENT';
      String content = getXml(files['ppt/slides/slide1.xml']!);

      expect(content, contains('<p:sld'));
      expect(content.indexOf(statement), -1);

      final slide = pres.slides[0] as SlideStatement;
      slide.statement = TextValue.uniform(statement);

      await save();
      content = getXml(files['ppt/slides/slide1.xml']!);

      expect(content.indexOf(statement), isNot(-1));
    });
    ...
    test('ppt/slides/_rels/slide1.xml.rels', () {
      ...
      expect(content, contains('slideLayout10.xml"'));
    });
